...
EVP_PKEY* STSMMgr::DHE_2048_Keygen()
 {
  ...
  if(EVP_PKEY_assign(DHParams, EVP_PKEY_DHX, DH_get_2048_256()) != 1)
   THROW_EXEC_EXCP(ERR_OSSL_EVP_PKEY_ASSIGN, OSSL_ERR_DESC);
  ...
  if(EVP_PKEY_keygen_init(DHGenCtx) != 1)
   THROW_EXEC_EXCP(ERR_OSSL_EVP_PKEY_KEYGEN_INIT, OSSL_ERR_DESC);

  // Generate an ephemeral DH 2048 key pair
  if(EVP_PKEY_keygen(DHGenCtx, &DHEKey) != 1)
   THROW_EXEC_EXCP(ERR_OSSL_EVP_PKEY_KEYGEN, OSSL_ERR_DESC);
...
void STSMMgr::logEDHPubKey(EVP_PKEY* EDHPubKey)
 {
  ...
  if(!EDHPubKeyBIO)
   LOG_EXEC_CODE(ERR_OSSL_BIO_NEW_FP_FAILED, OSSL_ERR_DESC);

  // Write the specified public key into the file BIO
  if(EVP_PKEY_print_public(EDHPubKeyBIO, EDHPubKey, 1, NULL) != 1)
   LOG_EXEC_CODE(ERR_OSSL_EVP_PKEY_PRINT_PUBLIC_FAILED, OSSL_ERR_DESC);
...
void STSMMgr::writeEDHPubKey(EVP_PKEY* EDHPubKey,unsigned char* addr)
 {
  ...
  if(PEM_write_bio_PUBKEY(EDHPubKeyBIO, EDHPubKey) != 1)
   THROW_EXEC_EXCP(ERR_OSSL_PEM_WRITE_BIO_PUBKEY_FAILED, OSSL_ERR_DESC);
...
void STSMMgr::deriveAES128SKey(unsigned char* skey)
 {
  ...
  if(!_myDHEKey)
   THROW_EXEC_EXCP(ERR_STSM_MY_PUBKEY_MISSING);
  if(!_otherDHEPubKey)
   THROW_EXEC_EXCP(ERR_STSM_OTHER_PUBKEY_MISSING);
  ...
  if(EVP_PKEY_derive_init(sSecretDerCTX) <= 0)
   THROW_EXEC_EXCP(ERR_OSSL_EVP_PKEY_DERIVE_INIT, OSSL_ERR_DESC);
  ...
  if(EVP_PKEY_derive_set_peer(sSecretDerCTX, _otherDHEPubKey) <= 0)
   THROW_EXEC_EXCP(ERR_OSSL_EVP_PKEY_DERIVE_SET_PEER, OSSL_ERR_DESC);
  ...
  if(EVP_PKEY_derive(sSecretDerCTX, NULL, &sSecretSize) <= 0)
   THROW_EXEC_EXCP(ERR_OSSL_EVP_PKEY_DERIVE, OSSL_ERR_DESC);
  ...
  */
  ...
  if(EVP_DigestInit(sSecretHashCTX, EVP_sha256()) <= 0)
   THROW_EXEC_EXCP(ERR_OSSL_EVP_DIGEST_INIT, OSSL_ERR_DESC);
