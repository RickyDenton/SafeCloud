...
/**
 * SafeCloud Execution Error Codes Declarations
 *
 * These errors cause the TCP connection between the SafeCloud
 * client and server, if present, to be aborted (and the
 * application to be terminated for errors of FATAL severity)
 */
...
enum execErrCode : unsigned char
 {
  ...
  // ------------------ Server Private Key Retrieval Errors ------------------ //
  ...
  // --------------------- Server Listening Socket Errors --------------------- //
  ERR_LSK_INIT_FAILED,
  ...
  // -------------------- Server Connection Sockets Errors -------------------- //
  ...
  // ------------- X.509 Certificates Store Initialization Errors ------------- //
  ...
  // -------------------- Client Connection Socket Errors -------------------- //
  ERR_CSK_INIT_FAILED,
  ERR_SRV_UNREACHABLE,
  ERR_CSK_CONN_FAILED,
  ...
  // ---------------  Connection-aborting Client Session Errors --------------- //
  ...
  // ---------------  Server Endpoint Parameters Common Errors --------------- //
  ERR_SRV_ADDR_INVALID,
  ERR_SRV_PORT_INVALID,

  // -------------------- Connection Sockets Common Errors -------------------- //
  ...
  // ------------------ Files and Directories Common Errors ------------------ //
  ERR_DIR_OPEN_FAILED,
  ...
  // EVP_DIGEST and EVP_SIGN errors
  ...
  // --------------------- SafeCloud Object States Errors --------------------- //
  ERR_CONNMGR_INVALID_STATE,
  ERR_AESGCMMGR_INVALID_STATE,

  // ---------------  Connection-aborting Common Session Errors --------------- //
  ...
  ERR_EXEC_UNKNOWN                // Unknown Error
 };
...
/**
 * Substitutes the appropriate THROW_EXEC_EXCP_ macro depending on the
 * number of arguments passed to the THROW_EXEC_EXCP variadic macro:
 *  - 1 argument  -> execErrCode only
 *  - 2 arguments -> execErrCode + additional description
 *  - 3 arguments -> execErrCode + additional description + error reason
 */
